...
_kubectl_manager_completions() {
    ...
    if [[ $cword -eq 1 ]]; then
        COMPREPLY=($(compgen -W "$kubectl_manager_commands" -- "$cur"))
        return
    fi
    ...
    case "${words[1]}" in
        versions)
            if [[ $cword -eq 2 ]]; then
                COMPREPLY=($(compgen -W "$versions_subcommands" -- "$cur"))
        ...
        configs)
            if [[ $cword -eq 2 ]]; then
                COMPREPLY=($(compgen -W "$configs_subcommands" -- "$cur"))
        ...
        clusters)
            if [[ $cword -eq 2 ]]; then
                COMPREPLY=($(compgen -W "$clusters_subcommands" -- "$cur"))
            fi
            ;;
        use)
            if [[ $cword -eq 2 ]]; then
...
" 2>/dev/null)
                ...
                if [[ -d "bin" ]]; then
                    installed_versions=$(ls bin/kubectl-* 2>/dev/null | sed 's/.*kubectl-//' | tr '\n' ' ')
                fi
                COMPREPLY=($(compgen -W "$installed_versions" -- "$cur"))
            elif [[ $cword -eq 3 || ($cword -eq 4 && "${words[2]}" == "--kubectl") ]]; then
                COMPREPLY=($(compgen -W "--kubectl" -- "$cur"))
            fi
            ;;
        run)
            ...
            if [[ $cword -ge 2 ]]; then
                ...
                if [[ $cword -eq 2 ]]; then
                    COMPREPLY=($(compgen -W "$kubectl_commands" -- "$cur"))
                else
